function [decodedFrame, newReferenceFrame,finalResidualBlock,modes,mvf] = encode(currentFrame, referenceFrame,frameName, colorspace,blockSize,tuSize,delta_iframe,delta_pframe,profile,searchStrategy,searchWindow,intraMode)
...
%     residualBlock = zeros(height,width,3);
    ...
    if strcmp(colorspace,'y')
        ...
        if strcmp(type,'iframe')
            ...
            [residualBlockLuma, mvf] = interPrediction(currentFrame, referenceFrame,blockSize,searchStrategy,searchWindow, intraMode);
        end
...
%       inverseQP
    ...
    %encode yuv sequence
        if strcmp(type,'iframe')
            ...
            [residualBlockLuma, mvf] = interPrediction(currentFrame(:,:,1), referenceFrame,blockSize,searchStrategy,searchWindow,intraMode);
